...
export default function AdminDashboard() {
  ...
  const loadVisitors = async () => {
    try {
      ...
      const hostEmail = admin?.email || "";
      const res = await api.get(`/visitor/visitorsbyHostEmail`, {
  ...
  const approveVisitor = async () => {
    try {
      const token = localStorage.getItem("token");
      const res = await api.post(`/visitor/approve/${approveDialog._id}`, {
        action: "APPROVED",
        expectedDuration,
      }, {
        headers: {
  ...
  const rejectVisitor = async () => {
    ...
    try {
      const token = localStorage.getItem("token");
      const res = await api.post(`/visitor/approve/${rejectDialog._id}`, {
        action: "REJECTED",
